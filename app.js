document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderHomeView() {
    ...
    if (state.currentPortal === "HealthyGamerGG") {
      ...
      archiveSubtitle.textContent =
        "34 Master Syntheses on Neurobiology, Emotion, Vedic Psychology & Executive Function";
    } else if (state.currentPortal === "Naval Ravikant") {
      archiveTitle.textContent = "Naval Ravikant Archive";
      archiveSubtitle.textContent =
        "22 Master Syntheses on Wealth, Leverage, Mental Models, First Principles & Peace";
    } else if (state.currentPortal === "Luminaries & Biopics") {
      archiveTitle.textContent = "Luminaries & Biopics Archive";
      archiveSubtitle.textContent =
        "15 Iconic Life & Mind Architectures (Musk, Jobs, Marcus Aurelius, Munger, da Vinci, Tesla, Feynman, etc.)";
    } else {
      archiveTitle.textContent = "The Unified Issue Archive";
      archiveSubtitle.textContent =
        "71 Master Syntheses across Psychology, Leverage, First Principles & Icon Biopics";
    }
    ...
    if (featuredArt && state.selectedPillar === "ALL" && !state.searchQuery) {
      ...
      const isBiopic =
        featuredArt.knowledge_base.includes("Biopic") ||
        featuredArt.id.startsWith("B");

      let badgeLabel = "🧘 DR. K MASTER ESSAY";
      let icon = "🧘";
      let tagTitle = "Integrative Triad";
      let tagSub =
        "Western Neuroscience + Vedic Psychology + Concrete Protocols";

      if (isNaval) {
        badgeLabel = "⚓ NAVAL RAVIKANT ESSAY";
        icon = "⚓";
        tagTitle = "First-Principles & Leverage";
        tagSub = "Specific Knowledge + Permissionless Leverage + Peace of Mind";
      } else if (isBiopic) {
        badgeLabel = "🌟 LUMINARY BIOPIC ESSAY";
        icon = "🌟";
        tagTitle = "Life Arc & Mental Models";
        tagSub = "Historical Breakthroughs + Non-Negotiable Mental Frameworks";
      }
        ...
        <div class="p-8 sm:p-12 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
          <div class="lg:col-span-8 space-y-6">
            <div class="flex items-center gap-3">
              <span class="badge-amber">${badgeLabel}</span>
          ...
          <div class="lg:col-span-4 flex items-center justify-center p-6 border border-subtle rounded-xl bg-secondary/40 text-center">
            <div class="space-y-3">
              ...
              <div class="font-serif text-sm font-semibold text-primary">${tagTitle}</div>
    ...
    filtered.forEach((art) => {
      ...
      const isBiopic =
        art.knowledge_base.includes("Biopic") || art.id.startsWith("B");
      ...
      card.className =
        "p-6 rounded-xl border border-subtle bg-card hover:border-amber-500/40 transition-all flex flex-col justify-between group cursor-pointer space-y-4";
      card.innerHTML = `
        <div class="space-y-3">
          <div class="flex items-center justify-between gap-2">
            <span class="badge-amber">${badgeText}</span>
  ...
  function openArticle(id) {
    ...
    parsedHtml = parsedHtml
      ...
      .replace(
        ...
        '<div class="callout-box callout-neuro"><strong>🧠 Western Neurobiology:</strong>',
      )
      .replace(
        /<strong>Yogic \/ Cognitive Perspective:<\/strong>/g,
        '<div class="callout-box callout-vedic"><strong>🧘 Vedic Psychology:</strong>',
      );
  ...
  brandLogo.addEventListener("click", (e) => {
    ...
    document.querySelectorAll(".portal-btn").forEach((b) => {
      if (b.getAttribute("data-portal") === "ALL")
        b.classList.add("bg-amber-500", "text-black", "active");
